...
void set_string_value2(JNIEnv *env, char *name, char *name2, char *value)
{
    ...
    else if(strcmp(name, "IPv4") == 0)
    {
        ...
        else if(strcmp(name2, "Netmask") == 0)
        {
            ...
            jmethodID LinkAddress_init = env->GetMethodID(LinkAddress, "<init>", "(Ljava/net/InetAddress;I)V");
            ...
            ip_string_to_bytes(value, s_netmask);
            jbyteArray array = env->NewByteArray(4);
            env->SetByteArrayRegion(array, 0, 4, (jbyte*)s_netmask);
            jobject inet_address = env->CallStaticObjectMethod(InetAddress, getByAddress, array);
            env->DeleteLocalRef(array);

            jobject link_address = env->NewObject(LinkAddress, LinkAddress_init, inet_address, range);

            jmethodID addLinkAddress = env->GetMethodID(LinkProperties, "addLinkAddress", "(Landroid/net/LinkAddress;)V");
