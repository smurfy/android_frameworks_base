...
public:
    ...
    int set(int type, struct timespec *ts);
    int clear(int type, struct timespec *unused);
    int waitForAlarm();
...
int AlarmImplTimerFd::clear(int type, struct timespec *unused)
{
    if (type > ANDROID_ALARM_TYPE_COUNT) {
        errno = EINVAL;
        return -1;
...
static void android_server_AlarmManagerService_clear(JNIEnv* env, jobject obj, jlong nativeData, jint type, jlong seconds, jlong nanoseconds)
{
    AlarmImpl *impl = reinterpret_cast<AlarmImpl *>(nativeData);
    struct timespec ts;
    ts.tv_sec = seconds;
